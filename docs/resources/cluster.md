...
- `edition` (String) Denotes the cluster's edition. Clusters in Cockroach Continuum organizations set an `edition`; clusters in organizations that are not on Continuum set a `plan` instead, so `edition` and `plan` cannot both be set. `STANDARD` clusters require a `serverless` block and `MISSION_CRITICAL` clusters require a `dedicated` or `host` block. Changing the edition of an existing cluster is not currently supported. Allowed values are:
  * STANDARD
  * MISSION_CRITICAL
- `host` (Attributes) Configures a host cluster, which provides dedicated hardware for virtual clusters. Host clusters are available only in Cockroach Continuum organizations. (see [below for nested schema](#nestedatt--host))
...
- `with_empty_ip_allowlist` (Boolean) Set to true to create the cluster with no default entries to the IP allowlist, blocking inbound connections until entries are added manually. This field is not used after cluster creation; to add new allowlist entries, configure your cluster as normal.
