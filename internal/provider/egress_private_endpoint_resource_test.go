	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			...
			if testCase.expectError == "" {
				...
				call := s.EXPECT().ListEgressPrivateEndpoints(gomock.Any(), clusterID, gomock.Any()).
					Return(&client.ListEgressPrivateEndpointsResponse{
						EgressPrivateEndpoints: []client.EgressPrivateEndpoint{testCase.endpoint},
					}, nil, nil).
					Times(3)

				s.EXPECT().ListEgressPrivateEndpoints(gomock.Any(), clusterID, gomock.Any()).
					Return(&client.ListEgressPrivateEndpointsResponse{
						EgressPrivateEndpoints: []client.EgressPrivateEndpoint{},
					}, nil, nil).After(call)
