...
func testCheckRestoreResource(restoreResourceName string, clusterResourceName string, restoreType string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if status == "SUCCESS" {
			// completed_at is only set when restore completes, so we check it's present in SUCCESS status
			...
			// crdb_job_id is set while the restore is running, so we can only guarantee it's set in SUCCESS status
