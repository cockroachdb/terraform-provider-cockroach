...
func convertObjectsToTerraform(objects *[]client.RestoreItem) []RestoreItem {
	...
	for i, obj := range *objects {
		...
		if i > 0 && (obj.Database != currentDB || objSchema != currentSchema) {
			// Finalize the previous group
			tfObjects = append(tfObjects, makeRestoreItem(currentDB, currentSchema, currentTables))
			currentTables = nil
		}

		currentDB = obj.Database
		currentSchema = objSchema

		if obj.Table != nil {
			currentTables = append(currentTables, *obj.Table)
		}
	}
...
func convertRestoreOptsToTerraform(opts *client.RestoreOpts) *RestoreOpts {
	...
	// Only create RestoreOpts if at least one field has a meaningful value
	// (preserve null when all fields are zero values from protobuf defaults)
	hasNonEmptyString := (opts.NewDbName != nil && *opts.NewDbName != "") ||
		(opts.IntoDb != nil && *opts.IntoDb != "")
	hasTrueBoolean := (opts.SkipLocalitiesCheck != nil && *opts.SkipLocalitiesCheck) ||
		(opts.SkipMissingForeignKeys != nil && *opts.SkipMissingForeignKeys) ||
		(opts.SkipMissingSequences != nil && *opts.SkipMissingSequences) ||
		(opts.SkipMissingViews != nil && *opts.SkipMissingViews) ||
		(opts.SchemaOnly != nil && *opts.SchemaOnly)

	if !hasNonEmptyString && !hasTrueBoolean {
		return nil
	}

	result := &RestoreOpts{}
	if opts.NewDbName != nil && *opts.NewDbName != "" {
		result.NewDBName = types.StringValue(*opts.NewDbName)
	}
	if opts.IntoDb != nil && *opts.IntoDb != "" {
		result.IntoDB = types.StringValue(*opts.IntoDb)
	}
	// Only set boolean fields if they're true (leave as null if false to match Terraform expectations)
	if opts.SkipLocalitiesCheck != nil && *opts.SkipLocalitiesCheck {
		result.SkipLocalitiesCheck = types.BoolValue(*opts.SkipLocalitiesCheck)
	}
	if opts.SkipMissingForeignKeys != nil && *opts.SkipMissingForeignKeys {
		result.SkipMissingForeignKeys = types.BoolValue(*opts.SkipMissingForeignKeys)
	}
	if opts.SkipMissingSequences != nil && *opts.SkipMissingSequences {
		result.SkipMissingSequences = types.BoolValue(*opts.SkipMissingSequences)
	}
	if opts.SkipMissingViews != nil && *opts.SkipMissingViews {
		result.SkipMissingViews = types.BoolValue(*opts.SkipMissingViews)
	}
	if opts.SchemaOnly != nil && *opts.SchemaOnly {
		result.SchemaOnly = types.BoolValue(*opts.SchemaOnly)
	}
	return result
}
